...
const getLotCategoryDetails = (lotSizes: any[] | undefined, lotSize: number, price: number) => {
  if (!lotSizes || lotSizes.length === 0) return null;

  const retailMin = lotSizes.find(l => l.applicationCategory.toLowerCase().includes("retail") && l.applicationCategory.toLowerCase().includes("min")) || lotSizes.find(l => l.applicationCategory.toLowerCase().includes("retail"));
  const retailMax = lotSizes.find(l => l.applicationCategory.toLowerCase().includes("retail") && l.applicationCategory.toLowerCase().includes("max"));
  const shniMin = lotSizes.find(l => l.applicationCategory.toLowerCase().includes("hni") && l.applicationCategory.toLowerCase().includes("s-hni") && l.applicationCategory.toLowerCase().includes("min")) || lotSizes.find(l => l.applicationCategory.toLowerCase().includes("shni"));
  const shniMax = lotSizes.find(l => l.applicationCategory.toLowerCase().includes("hni") && l.applicationCategory.toLowerCase().includes("s-hni") && l.applicationCategory.toLowerCase().includes("max"));
  const bhniMin = lotSizes.find(l => l.applicationCategory.toLowerCase().includes("hni") && l.applicationCategory.toLowerCase().includes("b-hni") && l.applicationCategory.toLowerCase().includes("min")) || lotSizes.find(l => l.applicationCategory.toLowerCase().includes("bhni"));

  const retailMinLot = retailMin ? retailMin.lots : 1;
  const retailMaxLot = retailMax ? retailMax.lots : 13;
  const shniMinLot = shniMin ? shniMin.lots : 14;
  const shniMaxLot = shniMax ? shniMax.lots : 67;
  const bhniMinLot = bhniMin ? bhniMin.lots : 68;

  return {
    retail: {
      minLots: retailMinLot,
      minShares: retailMin ? retailMin.shares : retailMinLot * lotSize,
      minAmount: retailMin ? retailMin.amount : retailMinLot * lotSize * price,
      maxLots: retailMaxLot,
      maxShares: retailMax ? retailMax.shares : retailMaxLot * lotSize,
      maxAmount: retailMax ? retailMax.amount : retailMaxLot * lotSize * price,
    },
    shni: {
      minLots: shniMinLot,
      minShares: shniMin ? shniMin.shares : shniMinLot * lotSize,
      minAmount: shniMin ? shniMin.amount : shniMinLot * lotSize * price,
      maxLots: shniMaxLot,
      maxShares: shniMax ? shniMax.shares : shniMaxLot * lotSize,
      maxAmount: shniMax ? shniMax.amount : shniMaxLot * lotSize * price,
    },
    bhni: {
      minLots: bhniMinLot,
      minShares: bhniMin ? bhniMin.shares : bhniMinLot * lotSize,
      minAmount: bhniMin ? bhniMin.amount : bhniMinLot * lotSize * price,
    }
  };
};
    ...
    <div className="min-h-screen max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* Lot Distribution Visual Cards */}
          {ipo.lotSizes && (
            (() => {
              const details = getLotCategoryDetails(ipo.lotSizes, ipo.lotSize, ipo.priceBandMax);
              if (!details) return null;
              return (
                <div className="p-5 rounded-xl bg-white border border-slate-200 shadow-sm space-y-4">
                  <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-indigo-700" />
                    IPO Lot Distribution &amp; Investor Brackets
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="p-4 rounded-xl bg-blue-50/50 border border-blue-100 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-slate-500 font-semibold">Max Cap: ₹2 Lakhs</span>
                      </div>
                      <div className="space-y-1">
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">Min Bid:</span>
                          <strong className="text-slate-800">{details.retail.minLots} Lot ({details.retail.minShares} Shares)</strong>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">Min Amount:</span>
                          <strong className="text-slate-900">₹{details.retail.minAmount.toLocaleString("en-IN")}</strong>
                        </div>
                        <div className="flex justify-between text-xs pt-1 border-t border-dashed border-blue-200">
                          <span className="text-slate-500 font-medium">Max Bid:</span>
                          <strong className="text-slate-800">{details.retail.maxLots} Lots ({details.retail.maxShares} Shares)</strong>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">Max Amount:</span>
                          <strong className="text-slate-900">₹{details.retail.maxAmount.toLocaleString("en-IN")}</strong>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-purple-50/50 border border-purple-100 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-slate-500 font-semibold">₹2L to ₹10L</span>
                      </div>
                      <div className="space-y-1">
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">Min Bid:</span>
                          <strong className="text-slate-800">{details.shni.minLots} Lots ({details.shni.minShares} Shares)</strong>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">Min Amount:</span>
                          <strong className="text-slate-900">₹{details.shni.minAmount.toLocaleString("en-IN")}</strong>
                        </div>
                        <div className="flex justify-between text-xs pt-1 border-t border-dashed border-purple-200">
                          <span className="text-slate-500 font-medium">Max Bid:</span>
                          <strong className="text-slate-800">{details.shni.maxLots} Lots ({details.shni.maxShares} Shares)</strong>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">Max Amount:</span>
                          <strong className="text-slate-900">₹{details.shni.maxAmount.toLocaleString("en-IN")}</strong>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-emerald-50/50 border border-emerald-100 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-slate-500 font-semibold">Above ₹10 Lakhs</span>
                      </div>
                      <div className="space-y-1">
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">Min Bid:</span>
                          <strong className="text-slate-800">{details.bhni.minLots} Lots ({details.bhni.minShares} Shares)</strong>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">Min Amount:</span>
                          <strong className="text-slate-900">₹{details.bhni.minAmount.toLocaleString("en-IN")}</strong>
                        </div>
                        <div className="flex justify-between text-xs pt-1 border-t border-dashed border-emerald-200">
                          <span className="text-slate-500 font-medium">Max Bid:</span>
                          <strong className="text-slate-800 font-medium">No Upper Limit</strong>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500 font-medium">Max Amount:</span>
                          <strong className="text-slate-800 font-medium">No Upper Limit</strong>
